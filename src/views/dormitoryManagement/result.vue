  ...
  <div>
    <div :class="{ 'box-loading': isLoading }" style="height: 100vh; overflow: auto">
      ...
      <div class="card">
        <div class="card-part">
          <van-row class="info-row">
            ...
            <van-col span="16" class="label-content">
              <div>
                <van-field disabled v-model="form.reason" row="3" autosize type="textarea" style="padding:0;" placeholder="请输入原因"/>
              </div>
            </van-col>
          </van-row>
        </div>
      ...
      <div class="card">
        <div class="card-part">
           <van-row class="info-row">
            ...
            <van-col span="16" class="label-content">
              <div>
                <label>{{ form.approvalResult == '1'?'同意' :'不同意' }}</label>
              </div>
            </van-col>
          </van-row>
          <van-row class="info-row">
            ...
            <van-col span="16" class="label-content">
              <div>
                <van-field disabled v-model="form.disagreementReasons" row="3" autosize type="textarea" style="padding:0;" placeholder="请输入原因"/>
              </div>
            </van-col>
          </van-row>
        </div>
      ...
      <footbar>
        <van-row>
          <van-col span="24">
            <van-button type="primary" style="height: 40px; width: 100%" @click="submitData">确认接收</van-button>
          </van-col>
        </van-row>
      </footbar>
    </div>
...
export default {
  ...
  methods: {
    submitData() {
      Dialog.confirm({
        ...
        .then(() => {
          ...
          request
            .post(API.roomAdjustConfirmReceiving, params)
            .then(res => {
              if (res.data.success) {
                alert('提交成功！请先退宿，才能安排住宿')
              } else {
    ...
    init() {
      ...
      request
        .get(API.roomAdjustGet, { params }) // 查询登录人信息
        .then(res => {
          let data = res.data
          if (data.success) {
            this.form = data.data || {}
            this.form = Object.assign(this.form, this.form.checkIn, this.form.bedMsg)
          } else {
            alert(res.data.msg)
          }
        })
