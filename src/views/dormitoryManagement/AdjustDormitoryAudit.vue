  ...
  <div>
    <div :class="{ 'box-loading': isLoading }" style="height: 100vh; overflow: auto">
      ...
      <footbar>
        <van-row>
          <van-col span="24">
            <van-button type="warning" style="height: 40px; width: 50%" @click="submitData('2')">不同意</van-button>
            <van-button type="primary" style="height: 40px; width: 50%" @click="submitData('1')">同意</van-button>
          </van-col>
        </van-row>
      </footbar>
    </div>
...
export default {
  ...
  methods: {
    submitData(val) {
      Dialog.confirm({
        ...
        .then(() => {
          ...
          params.approvalResult = val
          request
            .post(API.approval, params)
            .then(res => {
              if (res.data.success) {
                alert('提交成功！')
                this.$router.go(-1)
    ...
    init() {
      let params = {
        roomAdjustAppId: this.$route.query.roomAdjustAppId
      }
      request
        .get(API.getById, { params }) // 查询登录人信息
        .then(res => {
          let data = res.data
          if (data.success) {
            this.form = data.data || {}
